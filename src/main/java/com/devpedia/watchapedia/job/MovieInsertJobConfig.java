...
public class MovieInsertJobConfig {

    public static final String BASE_PATH = "movie/images/";
    ...
    @Bean
    public Job movieInsertJob() {
        return jobBuilderFactory.get("movieInsertJob")
                .start(movieInsertStep1())
                .build();
    }

    @Bean
    public Step movieInsertStep1() {
        return stepBuilderFactory.get("movieInsertStep1")
                .transactionManager(jpaTransactionManager())
                .<MovieInsertBatchDto, Movie>chunk(30)
                .reader(movieInsertReader())
                .processor(movieInsertProcessor())
                .writer(movieInsertWriter())
                .build();
    }

    @Bean
    public JsonItemReader<MovieInsertBatchDto> movieInsertReader() {
        return new JsonItemReaderBuilder<MovieInsertBatchDto>()
                .jsonObjectReader(new JacksonJsonObjectReader<>(MovieInsertBatchDto.class))
                .resource(new ClassPathResource("movie/movie_dump.json"))
                .name("jsonCursorItemReader")
                .build();
    }

    @Bean
    public ItemProcessor<MovieInsertBatchDto, Movie> movieInsertProcessor() {
        return batchDto -> {
            Movie movie = Movie.builder()
                    ...
                    .runningTimeInMinutes(batchDto.runningTimeInMinutes)
            ...
            for (Role role : batchDto.roles) {
                ...
                ContentParticipant cp = ContentParticipant.builder()
                        ...
                        .build();
                movie.addContentParticipant(cp);
            }
    ...
    @Bean
    public JpaItemWriter<Movie> movieInsertWriter() {
    ...
    @AllArgsConstructor
    @Builder
    public static class MovieInsertBatchDto {
